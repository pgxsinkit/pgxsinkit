...
Deno.serve(async (req: Request) => {
  ...
  // Per-worker wall-clock budget. The board overrides this (EDGE_WORKER_TIMEOUT_MS) because
  // board-stream holds durable-streams long-polls open continuously: at the stock 60s a long-poll
  // worker accumulates wall-clock and is recycled mid-cycle (the "early termination" log + a
  // read-path reconnect) about once a minute. A larger budget makes that churn rare. Unset → stock 60s.
