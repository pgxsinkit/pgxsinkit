...
- **semver tag** (e.g. `0.2.1`) → the exact `<tag>` at dist-tag `latest` (release parity).
...
`gh auth token` (needs `read:packages`). Preview with `DRY_RUN=1 bun run release:npm 0.2.1`.
