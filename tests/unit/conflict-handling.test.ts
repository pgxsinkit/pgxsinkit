...
// ADR-0029 D4: a must-refetch wipe TRUNCATEs the synced cache, which does NOT fire the per-row
// reconcile trigger — so an acked-delete journal row is not cleared by the wipe itself. Completeness
// rides the reconcile LOOP (`reconcileTable`'s `clearable_entities` pass), which clears the acked-delete
...
describe("truncate-wipe reconcile completeness (ADR-0029 D4)", () => {
  it("the reconcile loop clears an acked-delete journal row after a TRUNCATE wipe (trigger skipped)", async () => {
    ...
    // A synced row present (its INSERT fires the trigger while the journal is empty → no-op), plus an
    // acked-delete journal row for the same entity (as if a delete mutation was acked by the server).
    await drizzleOver(db)
      .insert(synced)
      .values({
        id: PROJECT_ID,
    ...
    await drizzleOver(db)
      ...
      .values({
        ...
        entityKeyJson: JSON.stringify({ id: PROJECT_ID }),
        mutationSeq: 1,
        mutationKind: "delete",
        status: "acked",
    ...
    const journalCount = async () =>
      (await drizzleOver(db).select({ c: count() }).from(journal).where(eq(journal["id"]!, PROJECT_ID)))[0]?.c ?? 0;
    expect(await journalCount()).toBe(1);

    // The must-refetch wipe: TRUNCATE the synced cache. A row-wise DELETE would fire the trigger and clear
    // the acked-delete journal row in real time; TRUNCATE deliberately does NOT, so the row still lingers.
    await drizzleOver(db).execute(sql`TRUNCATE ${synced}`);
    expect(await journalCount()).toBe(1);

    // The reconcile loop's clearable_entities pass completes the clear now that the synced row is gone.
    await runtime.reconcile("projects");
