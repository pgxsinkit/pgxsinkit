...
import { bigint, getTableConfig, pgSchema, uuid, varchar, type AnyPgTable } from "drizzle-orm/pg-core";

import { getOverlayTable } from "@pgxsinkit/client";
import { attachSyncRegistrySchema, defineSyncRegistry, defineSyncTable } from "@pgxsinkit/contracts";
...
describe("ephemeral cluster DDL (ADR-0021 §3)", () => {
  ...
  it("executes: the ephemeral cluster lands in pg_temp, the persistent one in the schema, and both work", async () => {
    ...
    await drizzleOver(db).insert(examOverlay).values({
  ...
  // ADR-0029 D1 / item 5 latent bug: `getSyncedLocalTable` (the runtime apply target) must render an
  // ephemeral entry BARE even when the registry lives in a NON-public local schema — its cluster is TEMP
  // in pg_temp (resolved via search_path), so a `schema.table` reference would target a durable schema
  // the temp table does not live in. The old `makeTable` qualified it by localSchema regardless of lifecycle.
  it("getSyncedLocalTable renders an ephemeral entry BARE under a non-public local schema, persistent qualified", () => {
    const appLocal = pgSchema("app_local");
    const ephemeralExam = defineSyncTable({
      tableName: "exam_answer",
      schema: appLocal,
      retention: "ephemeral",
      makeColumns: () => ({ id: uuid("id").primaryKey(), answer: varchar("answer", { length: 200 }) }),
    });
    const persistentNote = defineSyncTable({
      tableName: "note",
      schema: appLocal,
      makeColumns: () => ({ id: uuid("id").primaryKey(), body: varchar("body", { length: 200 }) }),
    });
    const schemed = attachSyncRegistrySchema(
      defineSyncRegistry({ exam: ephemeralExam, note: persistentNote }),
      "app_local",
    );

    // Ephemeral → bare (schema undefined), so the rendered name resolves to pg_temp via search_path.
    expect(getTableConfig(getSyncedLocalTable(schemed, "exam")).schema).toBeUndefined();
    // Persistent → schema-qualified with the registry's local schema.
    expect(getTableConfig(getSyncedLocalTable(schemed, "note")).schema).toBe("app_local");
  });
  ...
  it("buildDesyncTableSql clean-truncates a cluster's read cache for both retentions (ADR-0021 §2 desync)", async () => {
    ...
    await drizzleOver(db).insert(authorsOverlay).values({
