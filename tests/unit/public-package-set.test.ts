...
// The publish surface is exactly these four packages. Every other workspace member MUST set
...
// scripts/build-public-packages.ts hardcodes the same four; this test guards the package.json side
// independently, so the two cannot drift apart unnoticed.
const EXPECTED_PUBLIC = ["@pgxsinkit/client", "@pgxsinkit/contracts", "@pgxsinkit/react", "@pgxsinkit/server"];
...
describe("publish surface", () => {
  ...
  it("exposes exactly the four public packages", () => {
