...
describe("sync config contracts", () => {
  ...
  const makeStorageColumns = () => ({
    id: uuid("id").primaryKey(),
    label: varchar("label", { length: 40 }).notNull(),
  });

  it("carries a storage declaration through defineSyncRegistry (definition form) and reads it back", () => {
    const registry = defineSyncRegistry({
      tables: { thing: defineSyncTable({ tableName: "storage_thing", makeColumns: makeStorageColumns }) },
      storage: { backend: "idbfs", durability: "strict" },
    });
    expect(getSyncRegistryStorage(registry)).toEqual({ backend: "idbfs", durability: "strict" });
  });

  it("carries a partial storage declaration (durability only) through defineSyncRegistry", () => {
    const registry = defineSyncRegistry({
      tables: { thing: defineSyncTable({ tableName: "storage_thing_partial", makeColumns: makeStorageColumns }) },
      storage: { durability: "strict" },
    });
    expect(getSyncRegistryStorage(registry)).toEqual({ durability: "strict" });
  });

  it("a bare registry map carries no storage — reads undefined (resolves to defaults at the mint seam)", () => {
    const registry = defineSyncRegistry({
      thing: defineSyncTable({ tableName: "storage_thing_bare", makeColumns: makeStorageColumns }),
    });
    expect(getSyncRegistryStorage(registry)).toBeUndefined();
  });

  it("a definition form with no storage reads undefined", () => {
    const registry = defineSyncRegistry({
      tables: { thing: defineSyncTable({ tableName: "storage_thing_none", makeColumns: makeStorageColumns }) },
    });
    expect(getSyncRegistryStorage(registry)).toBeUndefined();
  });

  it("rejects an invalid storage.backend at defineSyncRegistry (fail closed at module-eval)", () => {
    expect(() =>
      defineSyncRegistry({
        tables: { thing: defineSyncTable({ tableName: "storage_bad_backend", makeColumns: makeStorageColumns }) },
        storage: { backend: "opfs-repacked" as unknown as "opfs" },
      }),
    ).toThrow(/invalid storage\.backend/);
  });

  it("rejects an invalid storage.durability at defineSyncRegistry (fail closed at module-eval)", () => {
    expect(() =>
      defineSyncRegistry({
        tables: { thing: defineSyncTable({ tableName: "storage_bad_durability", makeColumns: makeStorageColumns }) },
        storage: { durability: "default" as unknown as "relaxed" },
      }),
  ...
  it("validates the authClaim managed-field strategy (ADR-0026)", () => {
    ...
    // A managed field named by its SQL COLUMN NAME (`created_by`) rather than the drizzle PROPERTY KEY
    // (`createdBy`) is rejected at this boundary — the property key is canonical, no column-name
    // tolerance (ADR-0012). Both resolution paths (mutation stamp, Server-version resolve) assume it.
    expect(() =>
      withManaged({ column: "created_by", applyOn: ["create"], strategy: "authClaim", claimPath: ["sub"] }),
    ).toThrow(/names an unknown column/);
  });
