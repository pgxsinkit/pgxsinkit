 ...
 * `numeric` (precision/format), `bytea`, `interval`, enums, `json`/`jsonb`, and arrays — those
 * route to the `json` tier (if json-representable) or the always-correct `insert` floor.
 */
...
function isCopySafe(column: SyncColumnType): boolean {
  return !column.isArray && COPY_SAFE_BASE_TYPES.has(normalizeBaseType(column.sqlType));
}
