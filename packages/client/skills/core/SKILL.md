...
const overlay = getOverlayTable(registry, "report"); // typed by property key: overlay.id, overlay.overlayKind
...
— a non-public local schema _must_ read it via `getReadModelView` — and `_overlay` / `_mutations` /
