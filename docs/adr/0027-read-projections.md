...
## Addendum (2026-07-06): egress redaction on projections — `serverProjection`, `serverOnlyColumns`, and the mandatory posture

The first secure-delivery consumer (an "administered window" that must stream an item body while
stripping the answer keys per row) surfaced a v1 gap: a projection could not carry its own
`serverProjection.rowTransform`, so a projection over a redacted owner silently egressed **raw** owner
rows — the proxy resolves the transform strictly by shapeKey, and the projection's entry had none.

Two additions, one enforcement:

- **`serverProjection?: ServerProjectionSpec | "unredacted"`** — a projection may declare its own egress
  transform, resolved by its shapeKey and run on its egress path only (transform first, then column
  omission, same order as an owning entry).
- **`serverOnlyColumns?`** — owner column keys the transform must READ but that are not client shape
  (e.g. a `keys_withheld` control flag): added to the Electric fetch allow-list, visible to the
  transform, then stripped by the same egress omission pass — never reaching the client wire. Requires a
  `rowTransform` and a `columns` subset; must be disjoint from `columns` and the PK.
- **Inheritance is refused, and silence is refused too.** A projection never inherits the owner's
  transform — an inherited transform whose input column is absent from the projection's fetch list would
  read `undefined` and silently fail **open**, half-protection worse than none. Since a bare projection
  over a redacting owner is therefore a silent leak, `defineReadProjection` **throws at definition time**
  when the owner declares an egress `rowTransform` and the projection declares no posture: declare your
  own `serverProjection`, or opt out with the literal `"unredacted"` (raw egress as a visible, reviewed
  decision). `"unredacted"` over a transform-less owner is itself rejected, so a stale opt-out cannot
  pre-authorize a leak the day the owner grows a transform.

Alternative considered and rejected: **inherit the owner's transform by default** — zero-config for the
common case, but without knowing which columns an (opaque) transform reads, the framework cannot
guarantee its inputs are fetched, and a half-fed transform fails open while *appearing* protected. An
explicit posture is ceremony exactly once per projection; a silent leak is forever.
