...
registry by `pgxsinkit-generate` and installed as an ordinary migration. At startup the server checked
only that a function with the expected **signature** exists
(`verifyPlpgsqlBatchFunction` → `to_regprocedure(...)`). It could not tell whether the installed
**body** matches the registry the server is running.
...
2. **Enforce it at server startup, hard by default.** `verifyPlpgsqlBatchFunction` reads the comment and
   compares it to `expectedApplyFingerprint`. The new `applyFunctionDriftCheck` option on
   `createSyncServer` selects the behaviour on a confirmed mismatch:
   - `"error"` (**default**): throw, refusing to serve writes against a stale applier — consistent with
     the existing hard-fail signature check, and the whole point is to make a silent footgun loud.
   - `"warn"`: log and continue, for deploy orders that update the function a beat after the server.
   - `"off"`: skip the comparison.

   An **absent or unknown-format comment** (a function installed by an older pgxsinkit, or by hand) is
   never treated as a mismatch — it is reported as "cannot verify" and allowed through, so upgrading the
   server can never break an already-correct deployment.
...
- Until the function migrations are regenerated, existing deployments are unaffected: their unfingerprinted
  functions hit the "cannot verify" path (a one-time warning), and `sync:function:check` reports drift
  (it is intentionally **not** wired into the default `validate`/CI gate yet for that reason).
- Backwards compatible at runtime, opt-out by config; the only breaking-ish change is `sync:function:generate`
  now targeting the registry the server actually runs.
