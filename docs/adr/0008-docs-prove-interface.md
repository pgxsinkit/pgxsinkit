...
- **Decision 4 (`target: "bun"` investigation) — done; no change needed.** The built dist
  ...
  `target: "bun"` stays.
  - **Superseded for the React package (2026-07-11) by
    [ADR-0037](0037-vite-library-build-for-react-package.md).** The evidence above was gathered on
    `client`/`contracts`/`pglite-sync` and never covered `react`, and "no runtime-specific imports"
    turned out not to imply bundler-portability: `Bun.build` compiled the React package's JSX
    against `react/jsx-dev-runtime`, which downstream Vite production builds rewrite to
    `jsxDEV = undefined` — a render-time crash a Bun-run import smoke cannot see. `packages/react`
    now builds through Vite library mode; `target: "bun"` remains for the other packages, and the
    fixture gained a production Vite consumer that actually renders (ADR-0037 §4).
