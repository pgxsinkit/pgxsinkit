...
pgxsinkit is pre-launch (every publishable `package.json` is `0.0.0`, no external consumers), so removing
...
1. **Remove Hono from `@pgxsinkit/server`.** Replace it with a small internal `FetchRouter`
   ...
   (the `/api/*` prefix, the `/mutations` alias, and the shape-proxy path), and an error boundary. The
...
- **Breaking API change:** the `app` option and the `registerMutationRoute` export are gone. Acceptable
  pre-launch, and the replacement seam is strictly more capable.
