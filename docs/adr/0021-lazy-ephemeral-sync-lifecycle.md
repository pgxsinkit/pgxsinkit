...
- **`ephemeral` = the whole per-table cluster as `TEMP` / `pg_temp`**: the synced/overlay/journal tables +
  ...
  `packages/client/src/schema.ts`. **The one leak into durable space — the group's Electric subscription
  cursor and its `shape_row_tags` reason sets — is closed by [ADR-0042](0042-session-scoped-sync-metadata-for-ephemeral-groups.md),
  which places them in `pg_temp` too**, so a warm engine restart re-streams an ephemeral view from scratch
  instead of resuming a stale durable cursor over the recreated-empty TEMP cluster.
