...
- Evidence and regression guard: the `rls-read` track measures `baseline / shape-query / RLS` across
  ...
  `EXPLAIN ANALYZE` capture that proved the cause (committed in `4dad32e`, `f0cecf6`, `7f7a469`).
- Correctness after the rewrite: unit (244), the demo membership integration suite (32 —
  membership-fanout, asymmetric-read, write-state-gating), and the board smoke (5) all green; behaviour
  is unchanged. Landed in `e074e4b`.
