...
> **Historical record (obsolete).** The porting policy and refresh procedure below were retired by
> [ADR-0028](../adr/0028-own-the-sync-engine-outright.md): the suites are owned tests of our engine
...
1. **`useCopy` removed entirely; `'csv'` initial-insert method renamed to `'copy'`.** The deprecated
   ...
   registry-resolved `applyStrategy`. No deprecation window was needed at the time because there were no external consumers to keep
   compatible.
...
- **Metadata namespace (decision 6).** The local bookkeeping schema and the sync-origin GUC are
  ...
  internal local DBs existed before the supported `0.2.0` baseline, so the rebuild path handled the one-time schema change.
