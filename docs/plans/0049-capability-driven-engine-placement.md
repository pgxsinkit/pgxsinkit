...
record**, **commitment marker**, **provenance gate**, **backend permanence**, **destructive
lifecycle**. Plan and code comments use these terms exactly.
...
silent deletion of local-only data (a store's backend is fixed at first mint — no boot ever
replaces an existing store to change backends, D7); honest relocation outcomes (no blanket
"retryable", D10). The public surface this delivers: the registry storage declaration
(`storage.backend`/`storage.durability`), the factory-shaped attach input
(`worker: () => SharedWorker`) and the `createEngineWorker` override, `destroy()` on the attached
facade, `EngineRelocatedError` with outcome semantics, and the opt-in execution-limit configuration
(disabled by default).
...
3. **No stranded writes.** An uncommitted candidate is NEVER exposed to writes (commitment
   precedes exposure, D7); commitment forbids fallback once published.
4. **No silent local-only data loss.** A store's storage backend is FIXED at its first mint: an
   existing store is opened in place on the backend it was minted on, never replaced by a mint on
   another one, so no boot deletes local-only state to change backends — only a deliberate
   app-mediated destroy does (D7, D8). Restored stores are authoritative and never await a
   server (D7).
...
12. **The meta record is a total phase machine and first-use authority.** One phase
    (`idb-authoritative | opfs-candidate | opfs-committed | deleting`), `deleting`
    ...
    when it settles either inherited authority — completing a `deleting` handoff, or retiring an
    unexposed `opfs-candidate` — both before the replacement idb store is exposed. A failed meta read
    is an ERROR (bounded retry → fail closed), never "no record" (D6).
...
14. **Recordless idb stores are recognized.** No record + existing idb store (non-creating existence
    ...
    a fresh opfs mint; it is opened in place and stays idb-authoritative. This arm is a TERMINUS: the
    backend is fixed at first mint, so a home that later holds handles opens the idb store as it
    finds it, minting neither candidate nor sentinel (D6, D7).
...
- **Reconstructible cache state:** unactivated candidates and sentinel-less residue after
  authorized destruction. Delete-and-resynchronize is sufficient; do not add precise restoration
  machinery.
...
- **Store meta record**: dedicated small IndexedDB database; one record per store:
  `{ phase, updatedAt }` with phase-specific fields; written at the first grant-scoped capability
  boot and completed before exposure (a no-grant / registry-forced idbfs mint stays RECORDLESS
  until a pass must write authority); destruction deletes the record. Failed reads: bounded retry
...
- **Boot classification** (pure decision module, unit-tested):
  ...
  2. `opfs-committed` → open with bounded retries; any final failure HARD (including unreachable
     root).
  3. `opfs-candidate` → delete + rebuild fresh (an unexposed candidate has no authority).
  4. `idb-authoritative` → idb boot. TERMINAL: the backend is fixed at first mint, so this verdict
     is the same in every home, grant or no grant.
  5. No record, OPFS observable: sentinel present → committed (repair record → 2); candidate
     directory without sentinel → delete + rebuild; nothing present → **recordless-idb check** (6).
  6. **Recordless-idb check** (also when OPFS is NOT observable): non-creating idb existence check
     (`indexedDB.open` aborted on `upgradeneeded`; never `databases()`) — existing idb store →
     write record `idb-authoritative` (also terminal, per 4); nothing → virgin: create per placement
     (record first; when OPFS is unobservable the new store is idb — double loss is accepted risk).
...
Shared barrier for both: gate → explicit `strictSync()` returns with VFS health good → sentinel →
`opfs-committed` phase → expose. Barrier failure publishes nothing. An uncommitted candidate is
never exposed to writes (invariant 3). Both provenances publish a store's OWN first backend, which
is why neither needs the server: nothing is replaced (see "Backend permanence" below).

Fresh/restore crash rows:

| State at boot                                   | Verdict                                                                                                                                                                                                                                               |
| ----------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| No record, no directory, no sentinel            | virgin / recordless-idb check (classification 6)                                                                                                                                                                                                      |
...
survive into the rebuilt candidate's lifetime.

## Backend permanence (D7)

A store's storage backend is FIXED at its first mint, for the store's whole life. The two
directions, both decided by the classification above and neither costing a boot-path pass of its
own:

| Store as it exists                                                  | Boot's engine home  | Behaviour                                                                                                                                                                                                  |
| ------------------------------------------------------------------- | ------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| idb store (`idb-authoritative`, or recordless — classification 4/6) | HOLDS an OPFS grant | Opened in place on `idbfs`. No candidate, no sentinel, no successor. The granted lane's provision runs the SAME non-creating idb existence check and DECLINES, so no empty opfs sibling is ever pre-minted |
| `opfs-committed` store                                              | holds NO grant      | Typed refusal (`CommittedStoreUnreachableError`) before any mint — never an empty idb sibling at the same path. Provision declines for the same reason                                                     |

The one route to a store on a different backend is a deliberate, app-mediated destroy —
`destroyStoreArtifacts(storePath)` or `client.destroy()` (destructive lifecycle below) — followed
by a fresh boot that re-syncs from the server and mints on the then-best backend. Nothing is
carried across the two stores; the replacement is a fresh store with a fresh commitment.
...
| Existing idb store, boot in a home that HOLDS an OPFS grant              | Opened in place on `idbfs` — the backend is fixed at first mint. No candidate, no sentinel, nothing migrated; the granted provision lane declines on the same idb existence check rather than pre-mint an empty sibling                                                                                                                                                                                                                                                                                                                                          |
| `opfs-committed` store, boot in a home with NO grant                     | `CommittedStoreUnreachableError` before any mint (never an empty idb sibling); remedy is a granted home or a deliberate destroy                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| Recordless idb store, no meta record                                     | Non-creating existence check → `idb-authoritative`; never virgin-matched                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| Crash at each fresh/restore and destruction boundary                     | Per both crash tables — every row exercised                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| Provision while phase is `opfs-candidate` (granted or denied placement)  | Decline the pre-mint; only the adopting attach resolves a candidate — a granted boot rebuilds it, a denied boot retires it and publishes idb authority                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| Handle-denied boot finds `deleting`                                      | Delete old IDB honestly → remove the OPFS sentinel AND the store directory through async OPFS → write `idb-authoritative` → expose replacement IDB; if sentinel removal is unconfirmable, remain `deleting` and fail closed                                                                                                                                                                                                                                                                                                                                      |
| Handle-denied boot finds `opfs-candidate`                                | Retire the candidate: delete the sentinel (a barrier-gap crash can have published one) and the store directory through async OPFS → write `idb-authoritative` → expose the idb store. The retirement destroys the CANDIDATE only — it never deletes an IDB database at the path; an unobservable commitment namespace keeps candidate authority and fails closed                                                                                                                                                                                                 |
...
| opfs uncreatable, virgin                                                 | idbfs fallback after bounded retries: candidate torn down, record written `idb-authoritative` (first-use authority — identical to the no-handle virgin path), verbatim reason in diagnostics. The record makes it STICKY, exactly as backend permanence requires: the next boot classifies `boot-idb-authoritative` (classification 4) and opens the idb store — never a per-boot destructive re-probe, and opfs only via a deliberate destroy + a fresh boot (implementation decision 2026-07-21, superseding the earlier "non-sticky retry next boot" wording) |
...
- **Unit (bun, no real workers).** Coordinator claims lifecycle (provision expiry, last-claim
  ...
  disjointness incl. `foo` vs `foo.committed`); meta phase machine (classification 1–6, precedence,
  failed-read fail-closed, recordless-idb existence check + its terminus); fresh/restore and
  destruction state machines over their crash tables; the granted provision lane's decline over an
  existing idb store; `EngineRelocatedError` code+outcome round-trip.
- **Integration (Playwright, multi-tab).** Chromium + Firefox: leader kill → succession + queue
  ...
  refusal + success) and both backend-flip directions leaving zero cross-backend residue;
  offline-first-boot fresh commit; recordless-idb store recognition AND its permanence across a
  reload under a granted, sync-enabled home. NOT a CI lane
...
2. Store meta record module: phase machine, creation-time write, failed-read policy, non-creating
   idb existence check, boot classification 1–6 (pure, unit).
3. Fresh/restore and destruction state machines as pure modules over their crash tables;
   provenance gates + the strictSync barrier sequence (unit).
...
11. Client boot wiring: the fresh/restore commitment boot path in `createSyncClient` — the pre-mint
    phase machine, the mint, then the shared barrier at the local-init milestone (pre-expose) — plus
    the virgin-uncreatable session idbfs fallback, and the attach/engine-entry integration
    follow-ups the elected path needs (the documented `awaitOwnershipRelease` no-op default; a
    repeat control-port delivery replacing the old channel on SharedWorker reconstruction).
...
- All fault-matrix rows have a test or an explicit manual-verification note; both crash tables
  fully exercised.
...
- Offline-first-boot fresh stores and restored stores commit without server contact; no boot ever
  deletes local-only state to change a store's backend.
- A dispatched-mutation relocation demonstrably surfaces `outcome: "unknown"` (never auto-retry);
  recordless idb stores demonstrably classify `idb-authoritative` and demonstrably STAY there
  across a reload under a granted, sync-enabled home.
- Destroy/recreate works end-to-end on a committed store, leaving no residue on either backend;
  the SW-death lane recovers via the leader keepalive with a factory, and via leader-op without.
- ADR-0032/0048 amendment notes merged; decisions index regenerated; `docs/testing-strategy.md`
  updated; consumer runbook updated with `storageBackend` diagnostics and backend permanence.
