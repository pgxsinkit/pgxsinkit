...
and `tests/integration/pglite-sync-e2e.integration.test.ts` (since renamed per ADR-0028:
`tests/unit/sync-engine.test.ts`, `tests/integration/sync-engine-e2e.integration.test.ts`). See
[docs/history/pglite-sync-vendoring.md](../history/pglite-sync-vendoring.md) for the contract (stay green
...
- Rename the metadata schema (and therefore the `*.syncing` GUC) from `electric` to a
  pgxsinkit-owned name; update `dropReadCache`/wipe SQL accordingly. (Before the supported `0.2.0` baseline: no
