...
- Preserve the **faithful-apply** rule: a net `INSERT` stays a plain `INSERT` (not an upsert),
  so a genuine PK collision still surfaces; the Shape inbox tests pin this behavior.
...
- Preserve cross-table submission order by carrying the JSON array position with `WITH ORDINALITY`
  and ordering groups by their earliest position. `mutationSeq` is table-local and cannot provide a
  total order when parent and child journals both start at 1.
...
- **Write** (unit): the serialization gate (Phase 5) plus a test that set-based apply preserves
  the JSON submission order when table-local mutation sequences overlap, the `GREATEST` monotonicity,
  `auth.uid()`, and per-entity ack timestamps. **Write**
