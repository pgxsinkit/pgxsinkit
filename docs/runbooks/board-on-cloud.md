...
`board.cloud.env`, then `bun run board:cloud:deploy`. Roughly three of the steps are manual console
actions that cannot be scripted from this repo (project creation, the Electric source, `supabase link`).
...
- **Project URL** — `https://<ref>.supabase.co` → `BOARD_SUPABASE_URL`.
...
- **Frontend CORS** — `board-write` allows `http://localhost:5173` by default, which is the local Vite
  origin you're using against the cloud backend, so no extra config is needed. If you host the SPA
  elsewhere, set `BOARD_ALLOWED_ORIGINS` as a function secret to include that origin.
...
- **What can't be scripted** — project creation, the Electric source, and `supabase link` are
  account-scoped console/CLI actions; this repo's scripts own only the repeatable migrate/deploy/seed.
