...
[the acked ledger](/concepts/event-lane/#the-acked-ledger-keeping-an-acked-row-a-little-longer) closes:
...
<Aside type="note" title="A growing Outbox is the design working, not a leak">
  ...
  is designed to absorb offline weeks. Note also that `destroy()` refuses while a row still awaits a verdict, exactly as
  it refuses on owed mutations (a row the acked ledger is retaining has had its verdict, so it never blocks).
</Aside>
