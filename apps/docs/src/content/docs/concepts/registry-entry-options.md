...
contract the read path's control plane, the write API, and every client consume.

This page is the field-by-field reference. The [core-concept pages](/concepts/) are the mental
...
**What it achieves.** The Postgres table name, and the default for the synced/local table name and
the shape key. Most other names derive from it.
...
**What it achieves.** How rows reach the client: the shape key, an optional column projection, and
the rule that scopes which rows each caller syncs. That rule picks the shape's **tier** — declare
`rowFilter` for the private tier (the caller's verified claims are fused into the predicate) or
`scope` for the shared tier (the predicate is a function of scope values only, and a separate
entitlement set decides which of them a subject may read). Declaring both is refused at definition
time. The control plane compiles the predicate once per shape, at shape creation — not per request.

```ts
shape: {
  rowFilter: (columns) => ({
    // real, rename-safe Drizzle column objects; the subject rides as a typed value
    customPredicate: (claims) =>
      claims.sub
        ? p.in(columns.teamId, p.subquery(memberships.teamId, p.eq(memberships.memberId, claims.sub)))
        : DENY_ALL_PREDICATE,
    revision: "v1",
  }),
},
...
- **`rowFilter.customPredicate`** — returns a `Predicate` built with the
  [`p.*`](/api/contracts/variables/p/) builders, or `null` to bypass filtering (e.g. admin). It is an
  **AST**, not SQL text: values travel as typed JSON scalars, so there is nothing to escape, and each
  comparison is checked against the column's own TypeScript type. Subqueries must stay
  **uncorrelated** — the inner `where` reads the inner table's columns only. Return
  [`DENY_ALL_PREDICATE`](/api/contracts/variables/deny_all_predicate/) to make no rows visible. Must
  be **pure**: it is called fresh per shape creation, and probed with empty claims to detect
  claims-dependence.
- **`rowFilter.columns`** — restrict the columns the shape emits.
- **`rowFilter.revision`** — an opaque version tag for the `customPredicate` _body_ (a closure can't
  be hashed). **Bump it whenever you change the filter logic**, or the fingerprint won't shift and
  clients will serve the stale shape.
- **`scope` / `where`** — the shared tier. `scope` names the columns whose values key the shape
  family; the predicate is _generated_ as an `AND` of equalities over them (`NULL` is a legal scope
  value and compiles to `IS NULL`), so two shapes of a family cannot overlap. `where` is the optional
  **static** conjunct — identical for every subscriber, and therefore legal in either tier. Which
  scopes a subject holds is not a registry concern: the entitlement set is supplied server-side, to
  `createSyncServer`'s `readPath`.
- **`tableName` / `shapeKey`** — default to the entry's `tableName`; override `shapeKey` only to
  give several shapes over one physical table distinct keys (which is what `defineReadProjection`
  does).

**Refused at definition time:** a `rowFilter` that restricts nothing — no `customPredicate` and no
column allow-list. Such a filter asserts a restriction it cannot carry, and would be compiled into a
shape with no subject test at all. A shape that genuinely serves every row omits `rowFilter` entirely;
that is how it says so.

**When to use.** Almost always — the row filter is your read-side authorization. The
[read path](/concepts/read-path/) page covers the two tiers and fan-out filters in depth.
...
**What it achieves.** A per-row rewrite applied on the read path's egress — server authority, not
...
2. **Quiet members are affordable.** durable-streams carries up-to-date as a response header on a
   live request, and a long-poll timeout returns `204` with it set — so a quiet shape re-asserts
   freshness every poll cycle instead of parking the group behind a stale watermark. Don't contort a
   schema to keep a lookup table out of its natural group.
3. **Don't group "everything".** A group advances only as fast as its slowest member, so group scope
   should be the joined cluster, not the whole registry — unrelated clusters belong in separate groups
   (or stay singletons).
...
An ephemeral table's sync bookkeeping — its subscription cursor — is session-scoped too, held in
`pg_temp` alongside the rows it tracks. The guarantee that follows: a returning session always
re-streams an ephemeral view **from scratch**. Close the tab, come back, and the view
...
class binds, evaluated against named claims personas. Each cell hands your predicate the real read
predicate for those claims (`readPredicate` — exactly what the control plane compiles into the shape at
subscribe) and the entry table's RLS policies rendered to SQL text (`renderedPolicies`), so one predicate
can check **both** enforcement surfaces at once.

```ts
import { assertRegistryInvariant, deniesAllRows } from "@pgxsinkit/contracts";
...
fingerprints, it can see the `customPredicate` **body** — because it evaluates it — so it catches a
filter-logic change no hash can; the trade is that it sees exactly the personas you enumerate, and nothing else.
...
fingerprint here it can't see the `customPredicate` body — bump `rowFilter.revision` so a logic-only
