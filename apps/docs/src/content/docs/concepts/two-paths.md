...
they are not one channel. Assuming "Electric syncs both ways" is the most common modelling error.
...
```
client  →  write API  →  PostgreSQL
```

Local edits do **not** travel back through Electric. They are staged locally, flushed as a batch to
a typed write API, and applied to Postgres by a single in-database function. See
...
- **You cannot "write to Electric."** Electric is read transport only. A mutation that isn't sent to
  the write API never reaches Postgres, and therefore never comes back down the read path.
- **The loop closes through Postgres.** A local write becomes durable only once the write API applies
...
> Read from PGlite. Write through the write API. Never write to a synced table directly, and never
