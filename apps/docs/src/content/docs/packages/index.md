...
pgxsinkit ships as a set of focused packages. Most apps install `client`, `server`, `contracts`, and
`sync-engine`, plus `react` for React bindings.
...
| **`@pgxsinkit/sync-engine`** | transitively (a thin wrapper over the vendored sync adapter); rarely installed directly.                      | browser / PGlite |
| **`@pgxsinkit/react`**       | you want React hooks/bindings over the client.                                                                | React            |
| **`@pgxsinkit/pglite-sync`** | almost never directly — the vendored, hardened fork of `@electric-sql/pglite-sync` that the sync engine uses. | browser / PGlite |
...
- **Read path:** `@pgxsinkit/client` (via `@pgxsinkit/sync-engine` → `@pgxsinkit/pglite-sync`)
