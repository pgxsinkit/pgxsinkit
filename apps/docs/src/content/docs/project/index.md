...
| PGlite          | 0.5.8, aliased to the `@pgxsinkit/pglite` fork (`0.5.8-pgx.2`) | local client database (peer dependency). The fork carries a transaction-end sync fix `pglite-opfs-repacked`'s durability contract depends on.    |
