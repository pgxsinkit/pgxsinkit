...
| Circuits engine | `ghcr.io/pgxsinkit/electric-circuits/engine:sha-44357c1`       | ElectricSQL's Circuits engine, from the pgxsinkit fork. Pinned by **sha tag**, never `main`.                                                     |
| durable-streams | `ghcr.io/pgxsinkit/durable-streams-rust:sha-c269d9b`           | ElectricSQL's Rust durable-streams server, from the pgxsinkit fork. Pinned by **sha tag**, never `main`.                                         |
| PGlite          | 0.5.8, aliased to the `@pgxsinkit/pglite` fork (`0.5.8-pgx.1`) | local client database (peer dependency). The fork carries a transaction-end sync fix `pglite-opfs-repacked`'s durability contract depends on.    |
