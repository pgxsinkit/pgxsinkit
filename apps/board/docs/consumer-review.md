...
10. **Edge Functions prepend the function name to the request path; the mutation route
    does not expect it.** A POST to `/functions/v1/board-write/api/mutations` arrives at the worker as
    `/board-write/api/mutations`, while the server accepts only `/api/mutations`. → **resolved**:
    the adapter strips the function-name prefix before `server.fetch`.
11. **`verify_jwt` is a platform concept; on raw edge-runtime you verify in-function.**
    ...
    verify against the project's asymmetric JWKS, then hand the decoded claims (already
...
13. **Electric tags shape responses with a long, CDN-oriented `cache-control`** (`max-age`
    - `stale-while-revalidate`) that assumes a CDN keyed on the full URL. Behind a
      ...
      `no-store`") would save the next consumer the same dig.
