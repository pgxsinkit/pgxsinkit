...
- **Packaging is hybrid.** A gitignored `board.cloud.env` (from a committed `board.cloud.env.example`)
  ...
  create the Supabase project and create the Electric Cloud source on that project's database. The
  project ref in the env file is authoritative and every Supabase CLI mutation receives
  `--project-ref`; mutable CLI link state is deliberately ignored. CLI authentication comes from a
  project-account-specific personal access token in the gitignored env file rather than broken global
  profile state, allowing independent demo and downstream Supabase accounts.
...
- The docs site's Demo & harness page reflects ADR-0007's JWKS-based board authentication; a
  `JWT_SECRET` is not part of the Cloud board configuration.
