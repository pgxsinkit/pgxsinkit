...
CREATE POLICY "team_member_insert" ON "team_member" AS PERMISSIVE FOR INSERT TO "authenticated" WITH CHECK (EXISTS (SELECT 1 FROM jsonb_array_elements_text(case jsonb_typeof(nullif(current_setting('request.jwt.claims', true), '')::jsonb -> 'app_metadata' -> 'roles') when 'array' then nullif(current_setting('request.jwt.claims', true), '')::jsonb -> 'app_metadata' -> 'roles' else '[]'::jsonb end) AS r(role) WHERE r.role = 'admin'));--> statement-breakpoint
CREATE POLICY "team_member_update" ON "team_member" AS PERMISSIVE FOR UPDATE TO "authenticated" USING (EXISTS (SELECT 1 FROM jsonb_array_elements_text(case jsonb_typeof(nullif(current_setting('request.jwt.claims', true), '')::jsonb -> 'app_metadata' -> 'roles') when 'array' then nullif(current_setting('request.jwt.claims', true), '')::jsonb -> 'app_metadata' -> 'roles' else '[]'::jsonb end) AS r(role) WHERE r.role = 'admin')) WITH CHECK (EXISTS (SELECT 1 FROM jsonb_array_elements_text(case jsonb_typeof(nullif(current_setting('request.jwt.claims', true), '')::jsonb -> 'app_metadata' -> 'roles') when 'array' then nullif(current_setting('request.jwt.claims', true), '')::jsonb -> 'app_metadata' -> 'roles' else '[]'::jsonb end) AS r(role) WHERE r.role = 'admin'));--> statement-breakpoint
CREATE POLICY "team_member_delete" ON "team_member" AS PERMISSIVE FOR DELETE TO "authenticated" USING (EXISTS (SELECT 1 FROM jsonb_array_elements_text(case jsonb_typeof(nullif(current_setting('request.jwt.claims', true), '')::jsonb -> 'app_metadata' -> 'roles') when 'array' then nullif(current_setting('request.jwt.claims', true), '')::jsonb -> 'app_metadata' -> 'roles' else '[]'::jsonb end) AS r(role) WHERE r.role = 'admin'));--> statement-breakpoint
...
CREATE POLICY "team_insert" ON "team" AS PERMISSIVE FOR INSERT TO "authenticated" WITH CHECK (EXISTS (SELECT 1 FROM jsonb_array_elements_text(case jsonb_typeof(nullif(current_setting('request.jwt.claims', true), '')::jsonb -> 'app_metadata' -> 'roles') when 'array' then nullif(current_setting('request.jwt.claims', true), '')::jsonb -> 'app_metadata' -> 'roles' else '[]'::jsonb end) AS r(role) WHERE r.role = 'admin'));--> statement-breakpoint
CREATE POLICY "team_update" ON "team" AS PERMISSIVE FOR UPDATE TO "authenticated" USING (EXISTS (SELECT 1 FROM jsonb_array_elements_text(case jsonb_typeof(nullif(current_setting('request.jwt.claims', true), '')::jsonb -> 'app_metadata' -> 'roles') when 'array' then nullif(current_setting('request.jwt.claims', true), '')::jsonb -> 'app_metadata' -> 'roles' else '[]'::jsonb end) AS r(role) WHERE r.role = 'admin')) WITH CHECK (EXISTS (SELECT 1 FROM jsonb_array_elements_text(case jsonb_typeof(nullif(current_setting('request.jwt.claims', true), '')::jsonb -> 'app_metadata' -> 'roles') when 'array' then nullif(current_setting('request.jwt.claims', true), '')::jsonb -> 'app_metadata' -> 'roles' else '[]'::jsonb end) AS r(role) WHERE r.role = 'admin'));--> statement-breakpoint
CREATE POLICY "team_delete" ON "team" AS PERMISSIVE FOR DELETE TO "authenticated" USING (EXISTS (SELECT 1 FROM jsonb_array_elements_text(case jsonb_typeof(nullif(current_setting('request.jwt.claims', true), '')::jsonb -> 'app_metadata' -> 'roles') when 'array' then nullif(current_setting('request.jwt.claims', true), '')::jsonb -> 'app_metadata' -> 'roles' else '[]'::jsonb end) AS r(role) WHERE r.role = 'admin'));