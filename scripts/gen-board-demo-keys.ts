...
// (non-secret, local-only, and requiring rotation for any real use). This script regenerates that
// coherent set; paste its output into those two files.
//
...
// material (unlike Supabase's `add-new-auth-keys.sh`, which also includes symmetric key material).
// GoTrue still requires GOTRUE_JWT_SECRET to boot
