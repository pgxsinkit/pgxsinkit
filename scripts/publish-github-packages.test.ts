...
describe("targetVersion", () => {
  test("release-parity uses the tag verbatim", () => {
    expect(targetVersion("0.0.0", releaseCtx)).toBe("0.1.33");
  });

  test("dev channel anchors above the latest release, regardless of the 0.0.0 placeholder", () => {
    expect(targetVersion("0.0.0", devCtx)).toBe("0.1.33-dev.1782043909.abc1234");
  });

  test("dev channel never sorts below the latest release", () => {
    const v = targetVersion("0.0.0", devCtx);
    // nextPatch(latest 0.1.32) == 0.1.33, so the dev pre-release is strictly above the 0.1.32 release.
    expect(Bun.semver.order(v, "0.1.32")).toBe(1);
    expect(Bun.semver.order(v, "0.1.33")).toBe(-1);
  });
...
describe("pinSiblingDeps", () => {
  const version = "0.1.33-dev.1782043909.abc1234";
  ...
  test("pins same-scope peerDependencies to the exact version (the release back-fill bug)", () => {
    ...
    // release), because a pre-release dev version does NOT satisfy `>=0.0.12` under SemVer.
    const pkg: Manifest = {
      name: "@pgxsinkit/react",
      version: "0.0.0",
      peerDependencies: { "@pgxsinkit/client": ">=0.0.12" },
    };
...
describe("semver helpers", () => {
  test("nextPatch bumps the patch component", () => {
    expect(nextPatch("0.1.32")).toBe("0.1.33");
  });

  test("maxCore returns the greater of two cores", () => {
    expect(maxCore("0.0.0", "0.1.33")).toBe("0.1.33");
